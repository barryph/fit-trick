/**
 * Test helpers that prove a piece of code cannot depend on the host clock or
 * the host timezone.
 *
 * A process only exposes one timezone at a time, so "run the same assertion in
 * three timezones" alone cannot show that a function ignores the ambient one.
 * Swapping in a `Date` whose local-time and "now" entry points throw is a
 * positive proof: if the code under test still produces the expected result,
 * it derived everything from the arguments it was given.
 */
 ...
 *
 * Allowed: `Date.UTC`, `new Date(number)`, `new Date(string)`, and the `getUTC*`
 * accessors - none of these consult the host timezone.
 *
 * Forbidden (they all read the host timezone or the host clock):
